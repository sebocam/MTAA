...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 0K PERFECT")
                return
        else:
            now = int(time.time())
            validity = now + expires

        logging.info("  From: %s" % fromm)
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Request Too Bad It Doesnt Work")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            # logging.info("3Destination %s" % destination)
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 Nikto tu neni")
        else:
...
if __name__ == '__main__':
    ...
    logging.info('Hosting IP:'+ipaddress)
